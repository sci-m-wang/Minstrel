...
an already completed run whose detailed artifacts are complete, use its existing artifacts and
original manifests without restarting models or updating its frozen inputs to match newer delivery
instructions. If originals are incomplete and no backup exists, follow the replacement-run procedure
in `result-delivery.md`.
